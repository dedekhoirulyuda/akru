...
<header class="sticky top-0 z-30 flex items-center h-16 bg-white border-b border-slate-200 dark:bg-slate-900 dark:border-slate-800 px-4 lg:px-6 gap-4 text-slate-800 dark:text-slate-100">
    {{-- Mobile menu toggle --}}
    ...
    {{-- Night Mode & Light Mode Theme Switcher --}}
    <button @click="$store.theme.toggle()" 
            type="button" 
            id="theme-toggle-btn"
            class="p-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-amber-300 dark:hover:bg-slate-800 transition-all cursor-pointer flex items-center justify-center" 
            :title="$store.theme.mode === 'dark' ? 'Beralih ke Mode Terang (Light Mode)' : 'Beralih ke Mode Malam (Night Mode)'"
            aria-label="Ganti Tema Tampilan">
        <svg x-show="$store.theme.mode === 'dark'" class="w-5 h-5 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/>
        </svg>
        <svg x-show="$store.theme.mode !== 'dark'" class="w-5 h-5 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/>
        </svg>
    </button>

    {{-- Notifications --}}
    <a href="{{ route('workflow.index') }}" class="relative p-2 rounded-xl text-slate-500 hover:text-slate-900 hover:bg-slate-100 dark:text-slate-400 dark:hover:text-slate-200 dark:hover:bg-slate-800 transition-colors" title="Antrean Persetujuan">
