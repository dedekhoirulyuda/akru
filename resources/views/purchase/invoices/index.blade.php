...
@section('title', 'Tagihan Pembelian (Purchase Invoices) — AKRU')

@section('header')
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Tagihan Pembelian (Purchase Invoices)</h1>
        <p class="text-sm text-slate-500 mt-0.5">Pencatatan faktur pembelian dari pemasok, pengakuan hutang usaha & PPN masukan</p>
    </div>
    <div class="flex items-center gap-2">
        <x-export-dropdown 
            :excel-url="route('purchases.export.excel')" 
            :pdf-url="route('purchases.export.pdf')" 
            label="Export Tagihan" />
        <a href="{{ route('payments.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm font-medium hover:bg-slate-50 shadow-sm transition-colors">
        ...
        <a href="{{ route('purchases.create') }}" class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-500 shadow-sm transition-colors">
        ...
        <table class="w-full text-left border-collapse text-sm">
            <thead>
                <tr class="bg-slate-50/75 border-b border-slate-200 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    ...
                    <th class="py-3 px-6 w-36">No. Inv Pemasok</th>
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    <td class="py-3.5 px-6 font-mono font-medium">
                        <a href="{{ route('purchases.show', $inv) }}" class="text-blue-600 hover:text-blue-800 hover:underline">
                            {{ $inv->invoice_number }}
                        </a>
                    </td>
                    ...
                    <td class="py-3.5 px-6 text-right font-mono font-medium {{ $inv->remaining_amount > 0 ? 'text-amber-600' : 'text-emerald-600' }}">
                    ...
                    <td class="py-3.5 px-6 text-center">
                        <a href="{{ route('purchases.show', $inv) }}" class="text-xs text-blue-600 hover:text-blue-800 font-semibold">
                            Detail
                        </a>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="8" class="py-10 text-center text-slate-400">Belum ada tagihan pembelian tercatat.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
