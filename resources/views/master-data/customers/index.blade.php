...
@section('title', 'Daftar Pelanggan (Customers) — AKRU')

@section('header')
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-slate-900">Pelanggan (Customers)</h1>
        <p class="text-sm text-slate-500 mt-0.5">Master data rekanan bisnis pembeli dan profil perpajakan (NPWP/NIK)</p>
    </div>
    <div class="flex items-center gap-2" x-data="{ openModal: false, openImportModal: false }">
            ...
            <div class="flex items-center justify-center min-h-screen p-4 text-center">
                ...
                <div class="relative z-10 bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 text-left border border-slate-200">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                        <div>
                            <h3 class="text-lg font-bold text-slate-900">Impor Pelanggan dari Excel</h3>
                            <p class="text-xs text-slate-500 mt-0.5">Unggah data kontak pelanggan dalam jumlah banyak sekaligus via berkas Excel</p>
                        </div>
                    ...
                    <div class="mb-4 p-3.5 bg-blue-50/70 border border-blue-100 rounded-xl">
                        <div class="flex items-start gap-2.5">
                            ...
                            <div class="text-xs text-blue-900">
                                ...
                                <p class="text-blue-700 mt-0.5 font-mono text-[11px]">Kode Pelanggan, Nama Rekanan / Perusahaan, Jenis Identitas (NPWP/NIK), Nomor NPWP atau NIK, Email, Nomor Telepon / HP, Alamat Lengkap, Kota, Termin Pembayaran (Hari), Plafon Batas Kredit (Rp)</p>
                            </div>
                        </div>
                        <div class="mt-3 text-right">
                            <a href="{{ route('customers.template') }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white border border-blue-300 text-blue-700 text-xs font-semibold hover:bg-blue-50 shadow-xs transition-colors">
                    ...
                    <form method="POST" action="{{ route('customers.import') }}" enctype="multipart/form-data" class="space-y-4">
                        ...
                        <div class="flex justify-end gap-2 pt-3 border-t border-slate-100">
                            ...
                            <button type="submit" class="px-5 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-sm font-semibold shadow-sm transition-colors">Unggah & Impor Pelanggan</button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        {{-- Modal Tambah Pelanggan Manual --}}
        <div x-show="openModal" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
            <div class="flex items-center justify-center min-h-screen p-4 text-center">
                <div @click="openModal = false" class="fixed inset-0 bg-slate-900/60 transition-opacity"></div>
                <div class="relative z-10 bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 text-left border border-slate-200">
                    <h3 class="text-lg font-bold text-slate-900 mb-4">Tambah Pelanggan Baru</h3>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Kode Pelanggan</label>
                            <input type="text" name="code" placeholder="CUST-003" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Nama Rekanan / Kontak</label>
                            <input type="text" name="name" required placeholder="Nama lengkap atau PT" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <select name="identity_type" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500">
                                ...
                                <option value="NIK">NIK KTP (Perorangan)</option>
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="email" name="email" placeholder="billing@pelanggan.com" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500">
                        </div>
                        <div>
                            <label class="block text-xs font-semibold text-slate-700 mb-1">Telepon / WhatsApp</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-slate-700 mb-1">Alamat Penagihan & Faktur Pajak</label>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="city" placeholder="Jakarta Selatan" class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-blue-500">
                        </div>
                        <div>
                    ...
                    <div class="flex justify-end gap-2 pt-2">
                        ...
                        <button type="submit" class="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-500">Simpan Pelanggan</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <table class="w-full text-left border-collapse text-sm">
            <thead>
                <tr class="bg-slate-50/75 border-b border-slate-200 text-slate-500 text-xs font-semibold uppercase tracking-wider">
                    ...
                    <th class="py-3 px-6">Nama Rekanan</th>
            ...
            <tbody class="divide-y divide-slate-100 text-slate-700">
                ...
                <tr class="hover:bg-slate-50/80 transition-colors">
                    <td class="py-3.5 px-6 font-mono font-medium text-blue-600">{{ $cust->code ?? '-' }}</td>
                    <td class="py-3.5 px-6 font-medium text-slate-900">
                        {{ $cust->name }}
                        @if($cust->company_name && $cust->company_name !== $cust->name)
                            <span class="block text-xs text-slate-400 font-normal">{{ $cust->company_name }}</span>
                        @endif
                    </td>
                    <td class="py-3.5 px-6 font-mono text-xs text-slate-600">
                        {{ $cust->identity_number ? $cust->identity_number . ' (' . $cust->identity_type . ')' : '-' }}
                    </td>
                    <td class="py-3.5 px-6 text-xs text-slate-500">
                        <div>{{ $cust->phone ?? '-' }}</div>
                        <div class="text-slate-400">{{ $cust->city ?? '-' }}</div>
                    </td>
                    <td class="py-3.5 px-6 text-center">
                        <span class="inline-flex px-2 py-0.5 text-xs font-medium rounded bg-slate-100 text-slate-700">
                            {{ $cust->payment_terms_days > 0 ? 'Net ' . $cust->payment_terms_days : 'COD / Tunai' }}
                        </span>
                    </td>
                    <td class="py-3.5 px-6 text-center">
                ...
                @empty
                <tr>
                    <td colspan="6" class="py-8 text-center text-slate-400">Belum ada pelanggan terdaftar.</td>
                </tr>
                @endforelse
            </tbody>
        </table>
